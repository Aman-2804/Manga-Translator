...
function handleFiles(files) {
  ...
  if (hasPDF) {
    ...
    // Show loading spinner while we upload to get page previews
    dropZoneImages.innerHTML = `<div class="pdf-loading">Converting pages…</div>`;
    ...
    fetch(`${API}/upload`, { method: "POST", body: form })
      ...
      .then((upData) => {
        jobId = upData.job_id;
        if (upData.page_urls && upData.page_urls.length > 0) {
          originalUrls = upData.page_urls.map((u) => `${API}${u}`);
        }
        dropZoneImages.innerHTML = "";
        dropZoneTitle.textContent = `${upData.files_uploaded} page(s)`;
  ...
  if (!hasPDF) translateBtn.disabled = false;
}
...
translateBtn.addEventListener("click", async () => {
  ...
  translateBtn.disabled = true;
  translateBtn.textContent = jobId ? "Starting…" : "Uploading…";
  resultsSection.hidden = true;
  uploadSection.hidden = true;
  progressSection.hidden = true;

  try {
    // PDFs are pre-uploaded during preview — skip re-upload if we already have a jobId
    if (!jobId) {
      const form = new FormData();
      ...
      jobId = upData.job_id;
      if (upData.page_urls && upData.page_urls.length > 0 && originalUrls.every((u) => u === null)) {
        originalUrls = upData.page_urls.map((u) => `${API}${u}`);
      }
    }
