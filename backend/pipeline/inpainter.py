...
def _force_white_interior(image: np.ndarray, bubbles: list) -> np.ndarray:
    ...
    for b in bubbles:
        ...
        if is_dark:
            # For dark bubbles we already painted text pixels black; do a final
            # pass painting any remaining bright pixels black so nothing is missed.
            roi_gray = gray[y1:y2, x1:x2]
            bright = roi_gray > 80
            result[y1:y2, x1:x2][bright] = 0
        else:
            for sx, sy in seeds:
...
def inpaint_all_bubbles(
    ...
    if text_mask is not None and text_mask.shape[:2] == image.shape[:2]:
        ...
        for idx, bubble in enumerate(to_inpaint):
            ...
            if is_dark:
                # White text on black: directly paint all bright pixels black.
                # cv2.inpaint creates white splotches on dark backgrounds so we
                # skip it entirely — the background colour is known to be black.
                bx, by, bw, bh = bubble["bbox"]
                x1, y1 = max(bx, 0), max(by, 0)
                x2, y2 = min(bx + bw, image.shape[1]), min(by + bh, image.shape[0])
                gray_roi = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)[y1:y2, x1:x2]
                bright = gray_roi > 80
                image[y1:y2, x1:x2][bright] = 0
            else:
